...
HC05_StreamState HC05_Poll(void)
{
    while (UART5_RxAvailable() > 0U) {
        ...
        if (c == '\n') {
            ...
            else if (strcmp(cmd_buf, "CALIB_GYRO") == 0) { calib_cmd = HC05_CMD_CALIB_GYRO; UART5_WriteString("OK_CALIB_GYRO\r\n"); }
            else if (strcmp(cmd_buf, "CALIB_ACCEL") == 0) { calib_cmd = HC05_CMD_CALIB_ACCEL; UART5_WriteString("OK_CALIB_ACCEL\r\n"); }
            else if (strcmp(cmd_buf, "CALIB_ERASE") == 0) { calib_cmd = HC05_CMD_CALIB_ERASE; UART5_WriteString("OK_CALIB_ERASE\r\n"); }
